...
// Expanded endpoint list including high-availability mirrors
const OVERPASS_ENDPOINTS = [
  'https://overpass-api.de/api/interpreter',
  'https://overpass.kumi.systems/api/interpreter',
...
async function getLandCover(lat, lng) {
  ...
  // Optimized, compact Overpass QL query
  const query = `[out:json][timeout:5];(way["landuse"](around:300,${lat},${lng});way["natural"~"wetland|water"](around:300,${lat},${lng}););out tags;`;

  let landCover = 'Vegetated'; // Default safe assumption instead of throwing/failing
    ...
    try {
      const res = await fetchWithTimeout(endpoint, {
        ...
        headers: {
          ...
          'User-Agent': 'FloodGuard/1.0 (Contact: neuenhancedwebtechnologies@gmail.com)'
        }
      });

      if (!res.ok) continue; // Instantly skip bad endpoint (429/503)
      ...
      console.warn(`Landcover endpoint ${endpoint} unreachable:`, err.message);
    }
  }

  // Cache even default values to prevent cascading API overload on repeated coordinates
  await setCached('landcover', lat, lng, { landCover });
  return landCover;
}
